...
| **Trace (`-vvv`)**     | `src/inference_endpoint/utils/trace.py`, `utils/trace_dashboard.py`, `scripts/trace_dashboard.py` | Opt-in client-overhead tracer. Producers (main + worker procs) emit 17-byte binary frames (`<BQQ`: event/sid/ts) over a per-pid POSIX FIFO; a separate dashboard subprocess reads the FIFO + a dedicated ZMQ SUB to the aggregator PUB and renders a live `rich` view (request lifecycle, loadgen totals/latencies, event-loop lag). `trace.py` holds the lock-free SPSC ring emitter (drop-on-full, adaptive sampling) + FIFO/snapshot plumbing; `trace_dashboard.py` is pure aggregation/render (unit-tested in isolation); `PERF_START`/`PERF_END` bracket the perf window, and `teardown()` writes the terminal snapshot to the sidecar before closing the FIFO so the closing frame is authoritative.                                      |
...
└── utils/
    ...
    ├── trace.py               # -vvv trace: per-pid FIFO + 17-byte binary frame emitter, loop-lag/snapshot taps
    └── trace_dashboard.py     # -vvv trace: Dashboard aggregation + rich render (pure, no I/O; CLI entry in scripts/trace_dashboard.py)
