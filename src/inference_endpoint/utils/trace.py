...
Design
------
Each process (main + each worker) packs 17-byte events into a 512 MiB
...
* Kernel-buffered (``F_SETPIPE_SZ`` best-effort to 512 MiB) so the
  dashboard can block briefly without dropping data.
...
class Event(enum.IntEnum):
    """All known trace events. Adding one = one line here + wiring it into
    the dashboard's ``ingest_frames`` dispatch. The integer value goes on
    the wire, so changing existing numbers will break the dashboard — only
    ever append new ones with the next free value.
    """
    ...
    RESPONSE_DONE = 13
    # Emitted once by the main proc when session.run() returns — the
    # benchmark is over. The dashboard's authoritative "freeze" signal:
    # deterministic, unlike activity-idle (false-fires under drops) or the
    # loadgen snapshot state (false-stays-tail when the snapshot is stale).
    PERF_END = 14


# Bare-int aliases of the worker hot-path events. A plain int load at the call
# site skips the IntEnum member lookup (~50-100 ns) on every emit, even when
# tracing is off; the Event enum stays canonical for the wire docs and the
# dashboard. These MUST equal the Event values above.
E_WORKER_RECEIVED = int(Event.WORKER_RECEIVED)
E_CONN_ACQUIRED = int(Event.CONN_ACQUIRED)
E_WRITTEN = int(Event.WRITTEN)
E_RESPONSE_HEADERS = int(Event.RESPONSE_HEADERS)
E_RESPONSE_BYTES = int(Event.RESPONSE_BYTES)
E_RESPONSE_DONE = int(Event.RESPONSE_DONE)
...
def metrics_addr_path(pid: int) -> str:
    """Convention path for the aggregator's metrics-PUB ZMQ address.

    The dashboard subprocess reads this to open its OWN SUB socket and
    pull live snapshots straight from the aggregator — fresh even when the
    main loop is too saturated to advance its in-process subscriber.
    """
    return f"/tmp/endpoints_trace_{pid}/metrics_addr"


def publish_metrics_addr(addr: str) -> None:
    """Write the aggregator metrics-PUB address for the dashboard's SUB.
    No-op when tracing is off."""
    if not is_enabled():
        return
    try:
        with open(metrics_addr_path(os.getpid()), "w") as f:
            f.write(addr)
    except OSError:
        logger.debug("metrics addr publish failed", exc_info=True)
...
# FIFO pipe buffer (best-effort F_SETPIPE_SZ from both ends). Capped by
# /proc/sys/fs/pipe-max-size (1 MiB unprivileged → request fails, kernel
# default ~64 KiB kept); raise that sysctl (root) to actually grant 512 MiB.
...
def _next_sample_shift(
    shift: int, clear_ticks: int, new_drops: bool
) -> tuple[int, int]:
    """Drop-driven sampling policy for one tick. Raise the shift fast on any
    new drops (sample 2× harder, reset the clear counter); ease off one step
    only after ``_SAMPLE_RECOVER_TICKS`` of sustained no-new-drops — so it
    settles just above the drop knee. Pure so the state machine is testable
    without driving the async tick. Returns ``(shift, clear_ticks)``."""
    if new_drops:
        return min(shift + 1, _MAX_SAMPLE_SHIFT), 0
    clear_ticks += 1
    if clear_ticks >= _SAMPLE_RECOVER_TICKS and shift > 0:
        return shift - 1, 0
    return shift, clear_ticks


def emit_trace_id(event: int, req_id: str) -> int:
    """Emit by request UUID; returns the computed sid so a hot-path caller can
    reuse it across the request's later emits via :func:`emit_trace_sid`
    instead of re-parsing the UUID each time. Returns 0 when tracing is off
    (the no-op guard also makes non-hex ids like ``"q-1"`` safe in tests)."""
    if emit_trace is _noop:
        return 0
    sid = _sid_from_uuid(req_id)
    # Adaptive drop-driven sampling (only active once _sample_shift > 0).
    if _sample_shift and (sid & ((1 << _sample_shift) - 1)):
        return sid
    emit_trace(event, sid)
    return sid


def emit_trace_sid(event: int, sid: int) -> None:
    """Emit by a precomputed sid — skips the per-call UUID parse. The worker
    computes the sid once (from :func:`emit_trace_id`'s return) and reuses it
    across the per-request emits."""
    if emit_trace is _noop:
        return
    if _sample_shift and (sid & ((1 << _sample_shift) - 1)):
        return
...
async def emit_loop_lag(worker_id: int, period_s: float = 0.3) -> None:
    ...
    SPSC ring lock-free.
    """
    global _sample_shift
    _sample_shift = 0  # fresh per session (this task starts once per process)
    ...
    while True:
        try:
            ...
            if _active_emitter is not None:
                ...
                _sample_shift, clear_ticks = _next_sample_shift(
                    _sample_shift, clear_ticks, dropped > last_dropped
                )
...
def bootstrap(verbose: int) -> str:
    ...
    # Dashboard is alive — track it so cleanup() reaps it (a died one was
    # already reaped by the poll() above, so it needs no tracking).
    _state.dashboard_proc = dashboard_proc

    return _activate_trace(trace_dir, path)
...
class _BootstrapState:
    ...
    # The dashboard subprocess, so cleanup() can reap it (and kill a wedged
    # one as a backstop) rather than leaking it past the parent's exit.
...
# Grace cleanup() gives the dashboard to finish its final-frame render after
# it sees FIFO EOF before force-terminating a wedged one. wait() returns as
# soon as it exits (~1-2 s healthy), so this never truncates a live render.
_DASHBOARD_REAP_S = 10.0
...
def cleanup() -> None:
    ...
    if emitter is not None:
        ...
        try:
            ...
            pass  # already closed (pipe death / double cleanup)
    emit_trace = _noop
    ...
    proc = _state.dashboard_proc
    if proc is not None:  # pragma: no cover
        # Our FIFO write fd is now closed (workers closed theirs at exit), so
        # the dashboard sees EOF and exits after its final-frame render;
        # wait() returns the moment it does. terminate()/kill() only fire if it
        # has wedged — without this a stuck dashboard outlives the parent and
        # strands the terminal in the alternate-screen buffer.
        try:
            proc.wait(timeout=_DASHBOARD_REAP_S)
            ...
            pass  # already reaped
        _state.dashboard_proc = None

    if (
        _state.orig_stderr_fd is not None and _state.log_path is not None
    ):  # pragma: no cover
        try:
            os.write(
...
def _spawn_dashboard(pipe_path: str) -> subprocess.Popen | None:  # pragma: no cover
